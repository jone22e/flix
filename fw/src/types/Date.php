...
class Date {
    ...
    public function toStringBR() {
        ...
        if ($val=='') {
            ...
            $data = str_replace("-", "-", $val);
            $data1 = date('d', strtotime($data));
            $data2 = $meses[intval(date('m', strtotime($data)))];
            $data3 = date('Y', strtotime($data));
            return "{$data1} de {$data2} de {$data3}";
        }
    }
